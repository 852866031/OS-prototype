...
int exec(char *prog1, char *prog2, char *prog3){
    if(prog2!=NULL && strcmp(prog1, prog2)==0) {
        printf("Error: Script %s already loaded\n", prog1);
        return -1;    
    } 
    if(prog3!=NULL){
        if(strcmp(prog1, prog3)==0){
            printf("Error: Script %s already loaded\n", prog1);
            return -1; 
        }
        else if(strcmp(prog2, prog3)==0){
            printf("Error: Script %s already loaded\n", prog2);
            return -1; 
        }
    }
    if(prog1!=NULL) {
        if(myinit(prog1)<0) return -1; 
    }
    if(prog2!=NULL) {
        if(myinit(prog2)<0) return -1;  
    }
    if(prog3!=NULL) {
        if(myinit(prog3)<0) return -1;
    }
